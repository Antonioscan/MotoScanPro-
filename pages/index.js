...
export default function Home() {
  ...
  const analyzeImages = async () => {
    ...
    try {
      ...
      setResult(data.error ? { error: data.error, details: data.details } : { descrizione: data.descrizione });
    } catch (err) {
      console.error(err);
    ...
    <div className={styles.container}>
      ...
      {images.length > 0 && (
        <div className={styles.preview}>
          <img className={styles.mainImage} src={URL.createObjectURL(images[selectedIndex])} alt="anteprima" />
          <div className={styles.thumbGrid}>
            {images.map((img, i) => (
              <img key={i} src={URL.createObjectURL(img)} alt={`mini-${i}`}
                className={`${styles.thumbnail} ${i === selectedIndex ? styles.activeThumb : ''}`}
                onClick={() => setSelectedIndex(i)} />
            ))}
          </div>
          {images.length < 6 && (
            <button className={styles.addMore} onClick={openCamera}>+ Aggiungi foto</button>
          )}
        </div>
      )}

      <button className={styles.cta} onClick={analyzeImages} disabled={!images.length || loading}>
        ...
        <div className={styles.resultBox}>
          ...
          {result.descrizione ? (
            <>
              ...
              <button className={styles.copyBtn} onClick={() => navigator.clipboard.writeText(result.descrizione)}>
                Copia descrizione
              </button>
            </>
          ) : (
